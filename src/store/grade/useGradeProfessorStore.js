...
export const useGradeProfessorStore = defineStore("gradeProfessor", () => {
  ...
  const fetchGrades = async (lectureId) => {
    try {
      const res = await myAxios.get(`/api/professor/grades/lectures/${lectureId}`);
  ...
  const updateGradesStatus = async (lectureId, status) => {
    try {
      await myAxios.patch(`/api/professor/grades/lectures/${lectureId}/status?status=${status}`);
    } catch (error) {
  ...
  const replyObjection = async (gradeId, { approve, reply, newScores }) => {
    try {
      const url = `/api/professor/grades/${gradeId}/objection?approve=${approve}&reply=${encodeURIComponent(reply)}`;
